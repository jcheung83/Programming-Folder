...
class User:
    ...
    def validate( user ):
        ...
        if(not user['birthday'] or not user['first_name'] or not user['last_name'] or not user['email'] or not user['password']):
            flash("All fields are required.")
            is_valid = False
        
        if not user['first_name'].isalpha() or not user['last_name'].isalpha():
            flash("Invalid name, you cannot enter numbers or symbols in your name.")
            is_valid = False
        ...
        if age < 10:
            flash("User must be at least 10 years old to register.")
            is_valid = False
